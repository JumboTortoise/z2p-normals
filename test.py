...
def test(opts):
    ...
    test_export_dir = opts.export_dir
    ...
    test_loader = DataLoader(train_set, batch_size=opts.batch_size,
    ...
    avg_test_loss.reset()
    with torch.no_grad():
        for i, (img, zbuffer) in enumerate(test_loader):
            ...
            print(f'{run_name}; iter: {i}/{num_samples} loss: {test_loss.item()}')
            if global_step % opts.log_iter == 0:
                ...
                log_images(test_export_dir, f'pairs_iter_{i}', cat_img.detach())
            global_step += 1
...
if __name__ == '__main__':
    command = r"""--data /path/to/dataset --export-dir /where/to/save/output --batch-size 4 --num-workers 4 --epochs 10 --log-iter 1000 --losses masked_mse intensity masked_pixel_intensity masked_laplacan_weighted_mse --l-weight 1 0.7 1 1.2 """
